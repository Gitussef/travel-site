  ...
  calcSection(el) {
    if (
      ...
      if (
        ...
        let macthingLink = el.getAttribute("data-matching-link");
        document
          .querySelectorAll(`.primary-nav a:not(${macthingLink})`)
          .forEach((el) => el.classList.remove("is-current-link"));
        document.querySelector(macthingLink).classList.add("is-current-link");
      }
    }
  }
}
